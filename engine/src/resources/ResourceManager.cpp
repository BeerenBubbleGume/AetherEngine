...
namespace {
    ...
    auto trustedShaderProgram(
        ...
        switch (renderer) {
            ...
            case bgfx::RendererType::Direct3D12:
                ...
                program.vertex = engine::resources::embedded::basic_vs_win32;
                program.fragment = engine::resources::embedded::basic_fs_win32;
                break;
            case bgfx::RendererType::Metal:
                expectedVertex = assetsRoot / "shaders/bin/osx_arm/basic_vs.bin";
                expectedFragment = assetsRoot / "shaders/bin/osx_arm/basic_fs.bin";
                program.vertex = engine::resources::embedded::basic_vs_osx_arm;
                program.fragment = engine::resources::embedded::basic_fs_osx_arm;
                break;
            default:
                return std::nullopt;
        }
    ...
    auto createTrustedShader(
        ...
        if (!engine::security::validateShaderContainer(validationCopy, expectedStage, renderer)) {
...
namespace engine::resources {
    ...
    auto ResourceManager::loadMesh(std::string_view filename) -> MeshHandle {
        ...
        m_loadedMeshGroups += m_meshes.back().m_groups.size();
    ...
    auto ResourceManager::loadProgram(
        ...
        if (m_programs.size() >= MaxPrograms) {
        ...
        const auto id = static_cast<uint32_t>(m_programs.size());
