...
namespace {
    using engine::security::PathSecurityError;
    ...
    auto readFileByLockedPath(
    ...
    ) -> std::expected<engine::security::SecureFileContents, PathSecurityError> {
        auto locks = lockDirectoryChain(canonicalRoot, candidate.parent_path());
        if (!locks) {
            return std::unexpected(locks.error());
        }
        ...
        engine::security::SecureFileContents result;
...
auto engine::security::canonicalDirectory(const std::filesystem::path& directory)
    -> std::expected<std::filesystem::path, PathSecurityError> {
...
auto engine::security::normalizeRelativePathWithin(
    const std::filesystem::path& canonicalRoot,
    std::string_view requestedPath
) -> std::expected<std::filesystem::path, PathSecurityError> {
...
auto engine::security::resolveRegularFileWithin(
    const std::filesystem::path& canonicalRoot,
    std::string_view requestedPath
) -> std::expected<std::filesystem::path, PathSecurityError> {
...
auto engine::security::resolveDirectoryWithin(
    const std::filesystem::path& canonicalRoot,
    std::string_view requestedPath
) -> std::expected<std::filesystem::path, PathSecurityError> {
    const auto candidate = lexicalPathWithin(canonicalRoot, requestedPath);
...
auto engine::security::readRegularFileWithin(
    const std::filesystem::path& canonicalRoot,
    std::string_view requestedPath,
    std::uint64_t maximumBytes
...
auto engine::security::writeRegularFileWithin(
    const std::filesystem::path& canonicalRoot,
    std::string_view requestedPath,
    std::span<const std::uint8_t> contents,
    std::uint64_t maximumBytes
...
auto engine::security::isSafeSceneName(std::string_view sceneName) -> bool {
