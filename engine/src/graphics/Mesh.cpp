...
namespace {
    ...
    auto isValidGroup(const engine::graphics::MeshGroup& group) -> bool {
...
namespace engine::graphics {
    ...
    void Mesh::createFromVertices(std::span<const engine::math::Vec3> vertices, std::span<const uint16_t> indices) {
        destroyHandles();
    ...
    auto Mesh::loadFromBgfxGeometry(std::string_view filename) -> std::expected<void, MeshError> {
        auto bytes = readFileBytes(filename);
        if (bytes.empty()) {
            return std::unexpected(MeshError{1, "Failed to read mesh file"});
        }
